...
def summarize_all_pdfs():
    pdf_folder = os.path.join("extracted_data", "pdfs")
    summary_file = os.path.join(SUMMARIES_FOLDER, "pdf.txt")
    summaries = []
    ...
    for file_name in os.listdir(pdf_folder):
        ...
        # You must implement this: extract_pdf_content should get text content for summary
        # Assume extract_pdf_content returns an object with a 'texts' attribute (list of strings)
        pdf_content = extract_pdf_content(file_path)
        text_for_summary = "\n".join(pdf_content.texts)
        
        # Now summarize
        summary = summarize_text(text_for_summary)
        summaries.append(f"{file_name}:\n{summary}")

        print(f"Summarized PDF: {file_name}")

    # Write to summary file
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write("\n\n".join(summaries))

    print(f"All PDF summaries written to {summary_file}")

def summarize_all_texts():
    text_folder = os.path.join("extracted_data", "texts")
    summary_file = os.path.join(SUMMARIES_FOLDER, "text.txt")
    summaries = []
    ...
    for file_name in os.listdir(text_folder):
        ...
        print(f"Summarized Text File: {file_name}")

    # Write to summary file
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write("\n\n".join(summaries))

    print(f"All text summaries written to {summary_file}")
...
# Optional: Call both from main for convenience
